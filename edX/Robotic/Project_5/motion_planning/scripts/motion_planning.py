...
class MoveArm(object):
    ...
    def valid_point_func(self,q_start,q_goal):
        distance = 0.2
        startToGoal = q_goal - q_start
        normStg = numpy.linalg.norm(startToGoal)
        discPoints = numpy.outer(numpy.arange(distance,normStg,distance), startToGoal/normStg) + q_start

        pastPoint = [q_start]
        for discPoint in discPoints:
            if self.is_state_valid(discPoint) == False:
                return pastPoint
            pastPoint = numpy.array([discPoint])
    ...
    def motion_plan(self, q_start, q_goal, q_min, q_max):
        ...
        qStartArray = numpy.array([q_start],dtype=numpy.float64)
        qList = []
        qDistance = 0.5
        ...
        while True:
            ...
            if numpy.allclose(self.valid_point_func(qStartArray[-1], q_goal), numpy.array([q_goal])):
                nearestPoint = qStartArray[-1]
                qList.append(len(qStartArray)-1)
                qStartArray = numpy.append(qStartArray, numpy.array([q_goal]), axis=0)
                break

            #Generate Random Sample:
            qRandom = []
            for i in range(self.num_joints):
                qRandom.append(random.uniform(-3.1459, 3.1459))

            #Find Nearest Point
            qRandomArray = numpy.array([qRandom])
            diference = qStartArray - qRandomArray
            nodeClosest = numpy.argmin(numpy.sqrt(numpy.sum((diference)**2, axis=1)))
            nearestPoint = qStartArray[nodeClosest]

            #Point Lies in 0.25 Distance From Point to the Generated Point
            nearestToRandom = qRandomArray - nearestPoint
            nextPoint = nearestPoint + (nearestToRandom/(numpy.linalg.norm(nearestToRandom)))*qDistance

            #Return a Valid Point
            validPoint = self.valid_point_func(nearestPoint,nextPoint)

            #Append It To The Array
            qStartArray = numpy.append(qStartArray, validPoint, axis=0)
            qList.append(nodeClosest)

        qResult = []
        currentIndex = qList[-1]
        qResult.append(q_goal)
        for i in range(len(qStartArray)):
            if numpy.all(qStartArray[currentIndex] == q_start):
                qResult.append(q_start.tolist())
                break
            qResult.append(qStartArray[currentIndex].tolist())
            currentIndex = qList[currentIndex - 1]
